...
ENDPOINT = 'https://yg2xmxhcb27vthmrmed7zxxmui0qubjm.lambda-url.us-east-1.on.aws/'
...
def test_can_create_and_get_task():
    ...
    assert task['task']['user_id'] == user_id
    assert task['task']['content'] == random_task_content
...
def test_can_update_task():
    ...
    assert get_task_response.json()['task']["content"] == new_task_content
    assert get_task_response.json()['task']["is_done"] == True
